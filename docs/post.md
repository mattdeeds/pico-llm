...
I ran a 600M parameter LLM on a $1 microcontroller with only 520 KB of RAM. It's not fast generating one token every 15.5 seconds but its faster than you might initially think. I'd argue it's actually usable in certain scenarios. Someday the response from a super intelligent LLM may be more valuable than the hours/days/years it takes to generate that response. Sadly or thankfully we don't yet live in that future. Still, the idea of running a real LLM on something with the performance/power of your bluetooth speaker or smartbulb with no operating system, no frameworks, and no GPU is still something that consumed me. Here's how I did it.
