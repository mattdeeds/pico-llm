#!/usr/bin/env python3
"""Verify an exported Bonsai-1.7B Q1_0_g128 binary against HuggingFace output.

Reads Q1_0_g128 blocks from the binary, dequantizes per-block, runs a forward
pass matching Qwen3 (QK-Norm, GQA, split-half RoPE, SwiGLU), and compares
logits against the HuggingFace transformers model (prism-ml/Bonsai-1.7B-unpacked).

Because Bonsai is trained end-to-end at 1 bit, the Q1_0_g128 weights are the
"true" weights — the FP16-unpacked model stores the same values at full
precision. Expect high argmax agreement (bitwise-equivalent dequant, modulo
ordering and f32 rounding).
"""
...
def dequantize_q1_0_g128_matrix(data: bytes, rows: int, cols: int) -> np.ndarray:
    """Vectorized dequant: Q1_0_g128 bytes → float32 [rows, cols].

    Per-block layout (18 bytes, 128 weights):
      bytes[0:2]    float16 scale d
      bytes[2:18]   16 sign bytes; bit k of byte j → weight (j*8 + k)
                    bit = 1 → +d, bit = 0 → -d
    """
    assert cols % 128 == 0, f"cols={cols} must be divisible by 128"
    blocks_per_row = cols // 128
    row_bytes = blocks_per_row * 18
    assert len(data) == rows * row_bytes, \
        f"Expected {rows * row_bytes} bytes, got {len(data)}"

    # Reshape to [total_blocks, 18]
    arr = np.frombuffer(data, dtype=np.uint8).reshape(rows * blocks_per_row, 18)

    # Scales: first 2 bytes of each block → fp16 → fp32
    scales = np.frombuffer(arr[:, :2].tobytes(), dtype=np.float16).astype(np.float32)
    # shape: [total_blocks]

    # Sign bytes: bytes [2..18] → unpack to bits in little-endian order
    # (bit 0 of byte j is weight j*8, bit 1 is weight j*8+1, ...).
    # np.unpackbits default is bigendian (bit 7 first); we flip to little per byte.
    sign_bytes = arr[:, 2:]  # [total_blocks, 16]
    bits = np.unpackbits(sign_bytes, axis=1, bitorder="little")  # [total_blocks, 128]

    # Convert bits {0,1} → sign {-1, +1}
    signs = (bits.astype(np.int8) * 2 - 1).astype(np.float32)

    # Multiply by per-block scale
    weights = signs * scales[:, None]  # [total_blocks, 128]
...
class BinaryReader:
    ...
    def read_q1_0_g128_matrix(self, rows, cols):
        blocks_per_row = cols // 128
        row_bytes = blocks_per_row * 18
        total = rows * row_bytes
        blob = self.data[self.pos:self.pos + total]
        self.pos += total
        return dequantize_q1_0_g128_matrix(blob, rows, cols)


# ---------------------------------------------------------------------------
# Forward pass ops (Qwen3)
# ---------------------------------------------------------------------------
...
def forward_pass(binary_path, input_ids, rope_theta):
    ...
    print("Loading layer weights (Q1_0_g128 dequantization)...")
    layers = []
    for l in range(n_layers):
        ...
        layer["wq"] = r.read_q1_0_g128_matrix(q_dim, dim)
        layer["wk"] = r.read_q1_0_g128_matrix(kv_dim, dim)
        layer["wv"] = r.read_q1_0_g128_matrix(kv_dim, dim)
        layer["wo"] = r.read_q1_0_g128_matrix(dim, q_dim)
        layer["ffn_norm"] = r.read_float32_array(dim)
        layer["w_gate"] = r.read_q1_0_g128_matrix(hidden_dim, dim)
        layer["w_up"] = r.read_q1_0_g128_matrix(hidden_dim, dim)
        layer["w_down"] = r.read_q1_0_g128_matrix(dim, hidden_dim)
        layers.append(layer)
        print(f"  Layer {l + 1}/{n_layers}", end="\r")
    print(f"  Layer {n_layers}/{n_layers}")

    final_norm = r.read_float32_array(dim)
    print("Loading classifier (Q1_0_g128)...")
    wcls = r.read_q1_0_g128_matrix(vocab_size, dim)
...
def main():
    parser = argparse.ArgumentParser(
        description="Validate Bonsai-1.7B Q1_0_g128 binary against HuggingFace")
    parser.add_argument("binary", help="Path to exported Q1_0_g128 .bin file")
    parser.add_argument("--model-name", default="prism-ml/Bonsai-1.7B-unpacked",
    ...
    print(f"\nRunning Q1_0_g128 NumPy forward pass on {args.binary}...")
    ...
    if agree / total >= 0.75 and max_err < 20.0:
        print("\nPASS: Q1_0_g128 dequantization and forward pass match.")
